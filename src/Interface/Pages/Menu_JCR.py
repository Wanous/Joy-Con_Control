...
class RightJoyConConfigPage(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        """
        Page for the configuration of the Right JoyCon
        """
        super().__init__(parent, *args, **kwargs)

        self.master = parent
        self.JCRConfig = self.master.JCCHandler.OriginalConfig["JCR"]
    ...
    def _create_widgets(self):
        ...
        self.buttons = self.JCRConfig["Buttons"]
        self.buttons_frame = FrameButtons(self, "", self.buttons)
        self.buttons_frame_title = FrameTitle(self, titre="Joy con right buttons", couleur="#FF6F61", rayon=10, width=200,height=30, bg="#F0F0F0")

        # Frame for the joystick configuration
        self.joystick_frame = FrameJoystick(self, "R", titre="Joystick", bg="#F0F0F0", fg="white")
        self.joystick_frame_title = FrameTitle(self, titre="Right Joystick", couleur="#FF6F61", rayon=10, width=150,height=30, bg="#F0F0F0")
    ...
    def load_config(self, config, init = False):
        self.JCRConfig = config
        self.buttons_frame.load_config(self.JCRConfig["Buttons"])
        self.joystick_frame.load_config(self.JCRConfig["Joystick"])
